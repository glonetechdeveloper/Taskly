  ...
  function wireRoadmapOptionsMenu() {
      ...
      try {
        ...
        if (window.TasklyAPI && typeof window.TasklyAPI.addNotification === "function") {
          window.TasklyAPI.addNotification({
            ...
            type: "action",
            roadmap_id: currentActiveRoadmap.id
          });
        }
    ...
    regenBtn && regenBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (window.TasklyAPI && typeof window.TasklyAPI.addNotification === "function") {
          window.TasklyAPI.addNotification({
            message: `Regeneration started for "${currentActiveRoadmap.title || currentActiveRoadmap.goal_text || 'roadmap'}"`,
            type: "action",
            roadmap_id: currentActiveRoadmap.id
          });
        }
    ...
    confirmDeleteBtn && confirmDeleteBtn.addEventListener("click", async () => {
      ...
      try {
        const deletedTitle = currentActiveRoadmap.title || currentActiveRoadmap.goal_text || "roadmap";
    ...
    if (generateBtn) {
      generateBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          if (window.TasklyAPI && typeof window.TasklyAPI.addNotification === "function") {
            window.TasklyAPI.addNotification({
              message: `Created new roadmap: "${text.length > 35 ? text.substring(0, 32) + '…' : text}"`,
              type: "roadmap",
              roadmap_id: newId
            });
          }
